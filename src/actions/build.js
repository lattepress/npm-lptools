...
const buildReport = async (logs, reportDate) => {
  ...
  const dirName = `${reportDate.getFullYear()}/${reportDate.toLocaleString(
    'en-us',
    {month: 'short'},
  )}/${reportDate.getDate()}`
  ...
  if (!fs.existsSync(`reports/${dirName}`)) {
    fs.mkdirSync(`reports/${dirName}`, {recursive: true})
  }
...
Auto generated by "Report builder".
    `
  return writeFile(`reports/${dirName}/${fileName}`, txt, 'utf8').then(() => {
    console.log(`\n${txt}`)
    console.log('—————————————————————— End of report ——————————————————————')
    console.log(
      `Report file generated at ${__dirname}/reports/${dirName}/${fileName}`,
    )
  })
}
